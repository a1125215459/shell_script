...
case-proc_ams_agent_exist(){
	...
	proc_exist "/jffs2/accelbox/ams/ams" " /jffs2/accelbox/agent/agent-box"
}
...
case-executable_accelbox_init(){
	echo "/jffs2/bin/下编写一个可执行的accelbox_init.sh"
	sr 'rm -rf /jffs2/accelbox;
	rm -rf /jffs2/accelbox_bak;
	echo "echo 'sss' > /jffs2/test.log" > /jffs2/bin/accelbox_init.sh;
	chmod +x /jffs2/bin/accelbox_init.sh'
	rb
	sleep 150
	file_exist /jffs2/test.log
	file_not_exist /jffs2/accelbox
	del /jffs2/test.log
	del /jffs2/bin/accelbox_init.sh
	xy 3
	sleep 150
}
case-non_executable_accelbox_init(){
	echo "/jffs2/bin/随便手写一个无可执行权限的脚本（无任何操作）"
	del /jffs2/accelbox
	del /jffs2/accelbox_bak
	sr 'echo "echo 'sss' > /jffs2/test.log" > /jffs2/bin/accelbox_init.sh'
	rb
	sleep 150
	file_not_exist /jffs2/test.log
	file_exist /jffs2/accelbox
	a=`get_version "/jffs2/accelbox/version"`
	if [ $a = $versionC ];then
	echo "-------------->succ"	
	else
